...
def convert_motion(T, skel, translate_offset=None):
	# embed()
	# convert output array and gt array into fairmotion class
	# TODO set joint order
	if T is None:
		return None 
	
	motion = motion_classes.Motion.from_matrix(T, deepcopy(skel))
...
def divide_and_fk(seq, root_start_idx, rot_start_idx, rep, skel, parent, zero_root=None):
    ...
    if rep == "6d":
        # convert output
        denorm_rotations_pred_ = denorm_rotations_pred.reshape(batch, seq_len, -1, 6) # map to 6d [J, 6] format
...
def write_result_pkl(render_data_list, save_dir, filename="", custom_config=None, contact_manager=None):
    data = {}
    data['output'] = []
    data['gt'] = []
    # data['foot_est'] = []
    ...
    for rd in render_data_list:
        ...
        if rd.use_gt:
            ...
            data['gt'].append(None)
        # data['hand'].append(rd.other_est['hand'])
        # data['foot'].append(rd.other_est['foot'])
        # data['foot_est'].append(rd.foot_est)

    # data['output'] = np.array(data['output'])
    # data['gt'] = np.array(data['gt'])
    # data['foot_est'] = np.array(data['foot_est'])
    # embed()
...
def write_result(render_data, skel, base_directory, subdirectory, idx=0):
    ...
    for i in range(length):
        ...
        motion_output = motion_classes.Motion.from_matrix(output, skel_for_motion)
        motion_gt = motion_classes.Motion.from_matrix(gt, skel_for_motion)
...
def quat_to_T(quat, pos):
    """
    This function transforms quat + pos into transform matrix
    quat.shape = [#, J, 4]
    rootPos.shape = [#, J, 3] (# and J should be same as quat)
    return.shape = [#, J, 4, 4]
    """
    T = conversions.Ap2T(quat_to_angle_axis(quat), pos)

    return T

def quat_to_rotmat(quat):
    R = conversions.A2R(quat_to_angle_axis(quat))
    return R

def quat_to_T_motion(quat, root_pos):
    """
    This function transforms quat of all joints + root pos into transform matrix
...
def invert_T(T):
    ...
    R_trans = R.swapaxes(-2,-1)
    R_trans_p = R_trans @ p[...,np.newaxis]
    invT[..., :3, :3] = R_trans
    invT[..., :3, 3] = -1 * R_trans_p[...,0]
    return invT

def invert_T_torch(T):
    """
    T shape [..., 4, 4]
    return invT [..., 4, 4]
    """
    ...
    # Create an identity matrix of the same shape as T
    identity = torch.eye(4, device=T.device).expand_as(T)

    invT = T.clone()
    invT[...,:,:] = identity

    R_trans = torch.transpose(R, -2, -1)
    R_trans_p = torch.matmul(R_trans, p.unsqueeze(-1))
    invT[..., :3, :3] = R_trans
    invT[..., :3, 3] = -R_trans_p[..., 0]
    return invT

def rotmat_to_T_motion(rot, root):
    # seq_len, joint, _, _ = rot.shape
    seq_t_shape = rot.shape[:-2] + (1,1)
    seq_T = np.tile(np.eye(4)[np.newaxis, np.newaxis], seq_t_shape)
    seq_T[...,:3,:3] = rot
    seq_T[...,0,:3,3] = root
    return seq_T
...
def T_to_6d_rot(T):
    """
    input: batch of transformation matrix [..., 4, 4]
    output: batch of 6d rotation -> [..., 6]    
    """
    R_tensor = torch.tensor(T[...,:3,:3])
    Rsix = transforms.matrix_to_rotation_6d(R_tensor).numpy()
    return Rsix

def T_to_6d_and_pos(T):
    """
    input: batch of transformation matrix [..., 4, 4]
    output: batch of concatenated [6d rotation, 3d pos] -> [..., 9]
    """
    return np.concatenate((T_to_6d_rot(T), T[...,:3,3]), axis=-1)
...
def rot_matrix_fk_tensor(lrot, root_pos, offset, parents, return_T=False):
    gp, gr = [root_pos], [lrot[:,:,0]]
    ...
    if return_T:
        gr_rotmat = torch.stack(gr, dim=2)
        gp_pos = torch.stack(gp, dim=3).permute(0,1,3,2)
        return gr_rotmat, gp_pos
...
# Orient the data (map the pivot orientation to face z)
def rotate_at_frame(X, Q, skel):
    """
    Re-orients the animation data
