...
class Skeleton(object):
    ...
    def get_index_joint(self, key):
        if isinstance(key, list):
            return [utils.get_index(self.index_joint, k) for k in key]
    ...
    def get_parent_index_list(self):
        """
        returns list of parent joint index
        list[cur joint idx] = parent joint indx
        parent of root is marked as -1
        e.g.,  [-1, 0, 1, 2, 3 .. ]
        """
        parent = []
        for joint in self.joints:
            if joint.parent_joint is None:
                ...
                parent_name = joint.parent_joint.name
                parent_idx = self.get_index_joint(parent_name)
                parent.append(parent_idx)
        
        return parent

    def get_joint_offset_list(self):
        """
        returns list of current joint skeleton offset
        list[cur joint index] = skeleton offset
        """
        joint_offset_list = np.zeros(shape=(len(self.joints), 3))
        for idx, joint in enumerate(self.joints):
            if joint.parent_joint is not None:
                offset = conversions.T2p(joint.xform_from_parent_joint)
                joint_offset_list[idx] = offset
...
class Pose(object):
    ...
    def __init__(self, skel, data=None):
        ...
        if data is None:
            data = [constants.EYE_T for _ in range(skel.num_joints())] # eye_T is a func -> fix to EYE_T to get T-pose
