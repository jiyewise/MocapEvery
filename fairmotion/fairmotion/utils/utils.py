...
def run_parallel(func, iterable, num_cpus=20, **kwargs):
    ...
    func_with_kwargs = partial(func, **kwargs)
    with Pool(processes=num_cpus) as pool:
        results = list(tqdm(pool.imap(func_with_kwargs, iterable), total=len(iterable)))
    return results
...
def parse_motion(motion, start, end):
    cut_motion = copy.deepcopy(motion)
    cut_motion.poses = cut_motion.poses[start:end+1]
    return cut_motion
