...
def load(
					...
					for joint_idx, joint in enumerate(motion.skel.joints):
						for channel in joint.info["bvh_channels"]:
							...
							if channel in position_channels: 
								value = scale * value
								if joint_idx != 0: # ignore position channel values for other joints 
									value = 0
						...
						if channel in position_channels:
							T[:, joint_idx] = T[:, joint_idx] @ conversions.p2T(
								positions[
									...
									position_channels[channel],
									:,
								] # [F, 3] for each channel (xpos, ypos, zpos)
							) # [F, 4, 4] conversion result
						# operator @ : perform matrix multiplication for each 4x4 matrix in F
	...
	if fps is not None:
		motion = resample_by_time(motion, fps=fps)

	return motion


def _write_hierarchy(motion, file, joint, scale=1.0, rot_order="XYZ", tab="", _remove_joint="None"):
	...
	for child_joint in joint.child_joints:
		if _remove_joint in child_joint.name:
			continue
		child_joint_order = _write_hierarchy(
			motion, file, child_joint, scale, rot_order, tab + "\t"
		)
		joint_order.extend(child_joint_order)
	if len(joint.child_joints) == 0  or any(_remove_joint in _child.name for _child in joint.child_joints):
...
def sample(motion, freq):

	poses = []
	for i in range(motion.num_frames()):
		if i % freq == 0:
			poses.append(motion.get_pose_by_frame(i))
...
def resample_by_time(motion, fps):
	time_length = motion.length()
	fps_inv = 1.0 / fps
	time_count = 0.0
	resampled_poses = []
	while True:
		if time_count > time_length:
			break
		resampled_poses.append(motion.get_pose_by_time(time_count))
		time_count += fps_inv
	motion.poses = resampled_poses
	return motion
