...
def main(args):
    ...
    motions = []
    ref_files = utils.files_in_dir(args.result_dir + "ref/")
    pred_files = utils.files_in_dir(args.result_dir + "pred/")
    for ref, pred in zip(ref_files, pred_files):
        ref_filename = ref.split("/")[-1]
        pred_filename = pred.split("/")[-1]
        if ref_filename == pred_filename:
            ref_motion = bvh.load(
                file=ref,
                v_up_skel=v_up_env,
                v_face_skel=utils.str_to_axis(args.axis_face),
            ...
            # motion_ops.translate(pred_motion, [1,0,0])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--thickness", type=float, default=0.02,
