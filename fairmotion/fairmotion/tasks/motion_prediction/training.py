...
def train(args):
    ...
    utils.log_config(args.save_model_path, args) # write config to config.txt

    # save log (summary writer)
    log_dir = args.save_model_path + "/log/"
    ...
    # for training, mean and std is calculated
    # dataset = dictionary of train-dataloader, test-dataloader validation-dataloader

    # Loss per epoch is the average loss per sequence
    # length of dataloader is equal to the number of batches
    ...
    # shape is (batch_size, seq_len, num_predictions) [64, 24, 72]
    # next(iter(dataset["train"])) returns [batch of src, batch of tgt]
    ...
    model = utils.prepare_model(
        input_dim=num_predictions, # the dimension of each sequence -> 3J
        hidden_dim=args.hidden_dim, # dimension of hidden layer. this case 1024 (default args)
        device=device,
    ...
    epoch_loss = 0
    logging.info("Init...")
    for iterations, (src_seqs, tgt_seqs) in enumerate(dataset["train"]): # iterate for a number of batches
        model.eval()
    ...
    for epoch in range(args.epochs):
        ...
        step_per_epoch = len(dataset["train"])
        for iterations, (src_seqs, tgt_seqs) in enumerate(dataset["train"]):
            ...
            ) # outputs shape [batch, tgt_seq, 3J]
            outputs = outputs.double() 
            loss = criterion(
                outputs,
                utils.prepare_tgt_seqs(args.architecture, src_seqs, tgt_seqs), # for seq2seq architecture, [batch, tgt_seq, 3J]
            )
            
            writer.add_scalar('mse_loss', loss.item(), global_step = epoch * step_per_epoch + iterations)
        ...
        logging.info(				
                # forward kinematics
				# denormalize output
				# get skel from batch and do forward kinematics (before, normalize quaternions)
				
				# check whether denormalizing works well - compare seq from getitem->denormalize and from self.dataset.data['seq'][0] (idx is set to 0 in debug mode)
				# check whether forward kinematics works well

				# denorm target -> this is to check whether the functions are working well
				# denorm_seq = self.dataset.denormalize(tgt_seq, self.device)
				# denorm_rotations = denorm_seq[...,4+3:]
				# denorm_root_pos =  denorm_seq[...,4:4+3]
				# batch, seq, _ = denorm_rotations.shape
				# denorm_rotations_ = denorm_rotations.reshape(batch, seq, -1, 4) # map to quaternions
				# tgt_pos = quat_fk_tensor(denorm_rotations_ , denorm_root_pos, self.skel , self.config['skel']['parents']) # FK
            f"Training loss {epoch_loss} | "
            f"Validation loss {val_loss} | "
