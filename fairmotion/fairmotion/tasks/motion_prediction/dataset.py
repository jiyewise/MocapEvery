...
class Dataset(data.Dataset):
    def __init__(self, dataset_path, device, mean=None, std=None):
        ...
        # len(self.src_seqs) == len(self.tgt_seqs) 33526
        # shape of self.src_seqs[0] [120, 72]
        # shape of self.tgt_seqs[0] [24, 72]
        if mean is None or std is None:
            self.mean = np.mean(self.src_seqs, axis=(0, 1)) # shape of mean [72,]
            self.std = np.std(self.src_seqs, axis=(0, 1))
        else:
            self.mean = mean
            self.std = std
        self.num_total_seqs = len(self.src_seqs)
        print(self.num_total_seqs)
        self.device = device
