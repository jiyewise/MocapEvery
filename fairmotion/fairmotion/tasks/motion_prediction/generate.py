...
def generate(model, src_seqs, max_len, device):
    ...
    model.eval()
    with torch.no_grad():
        tgt_seqs = src_seqs[:, -1].unsqueeze(1) # src seq [64, 120, 72] -> [64, 72] (get the last frame) -> [64, 1, 72] by unsqueeze.
        # start with the last frame of the src
        # initial tgt_seqs size is [64, 1, 72]
