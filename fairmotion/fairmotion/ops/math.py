...
def R_from_vectors_tensor(vec1, vec2):
    """
    Find the rotation matrix that aligns x to y
    ...
    angle = np.arccos(np.clip(np.sum(vec1 * vec2, axis=-1) / (np.linalg.norm(vec1, axis=-1) * np.linalg.norm(vec2, axis=-1)), -1.0, 1.0))
    
    # Step 3: Find the rotation matrix using Rodrigues' formula
    axis_norm = np.linalg.norm(axis, axis=-1, keepdims=True)
    axis_normalized = axis / axis_norm
        
    K = np.zeros((vec1.shape[0], 3, 3))
    K[:, 0, 1] = -axis_normalized[:, 2]
    K[:, 0, 2] = axis_normalized[:, 1]
    K[:, 1, 0] = axis_normalized[:, 2]
    K[:, 1, 2] = -axis_normalized[:, 0]
    K[:, 2, 0] = -axis_normalized[:, 1]
    K[:, 2, 1] = axis_normalized[:, 0]
    ...
    # R = I + np.sin(angle)[..., np.newaxis, np.newaxis] * K + (1 - np.cos(angle))[..., np.newaxis, np.newaxis] * np.matmul(K, K)
    
    return R
    return 

def R_from_vectors_torch(vec1, vec2):
    import torch
    ...
    # If vectors are antiparallel, return the identity matrix
    ...
    v_x = torch.tensor([[  0, -v[2],  v[1]],
                        [ v[2],    0, -v[0]],
                        [-v[1],  v[0],   0]])

    # Rodrigues' rotation formula
    R = torch.eye(3) + v_x + torch.matmul(v_x, v_x) * ((1 - c) / s**2)
