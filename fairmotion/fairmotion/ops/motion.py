...
def adjust_height(motion, lfoot="LeftToe", rfoot="RightToe", height_axis="y", pivot=None):
    y_or_z = 1 if height_axis == "y" else 2

    if pivot is None:
        # get position sequence of left foot
        # get position sequence of right foot
        # get minimum
        joint_positions = motion.positions(local=False)
        foot_height = joint_positions[:,[lfoot_idx, rfoot_idx], y_or_z]
        min_foot_height = np.min(foot_height)

    else:
        lfoot_pos = conversions.T2p(motion.poses[pivot].get_transform(key=lfoot, local=False))
        lfoot_height = lfoot_pos[y_or_z]        

        rfoot_pos = conversions.T2p(motion.poses[pivot].get_transform(key=rfoot, local=False))
        rfoot_height = rfoot_pos[y_or_z]        
        min_foot_height = min(lfoot_height, rfoot_height)        
...
# def adjust_height(motion, lfoot="LeftToe", rfoot="RightToe", height_axis="y"):
#     y_or_z = 1 if height_axis == "y" else 2
#     # get position sequence of left foot
#     # get position sequence of right foot
#     # get minimum
#     joint_positions = motion.positions(local=False)
#     lfoot_idx, rfoot_idx = motion.skel.get_index_joint(lfoot), motion.skel.get_index_joint(rfoot)
#     foot_height = joint_positions[:,[lfoot_idx, rfoot_idx], y_or_z]
#     min_foot_height = np.min(foot_height)

#     # add offset so the minimum would be zero
#     return translate(motion, [0,-1*min_foot_height, 0]), -1*min_foot_height