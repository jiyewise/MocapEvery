...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.5, max_len=5000): # d_model: ninp in original
...
class TransformerModel(nn.Module):
    def __init__(
        ...
        # ntoken: 3J for aa representation (this is the dimension of the input)
        # ninp: 1024 this is the dimension of the hidden layer
        # num_heads: fixed to 4 in this case
    ...
    # this function masks the future state of the decoder
    def _generate_square_subsequent_mask(self, sz):
        mask = (torch.triu(torch.ones(sz, sz)) == 1).transpose(0, 1) # upper triangular
        mask = (
            mask.float()
            .masked_fill(mask == 0, float("-inf"))
            .masked_fill(mask == 1, float(0.0))
    ...
    def forward(self, src, tgt, max_len=None, teacher_forcing_ratio=None):
        ...
        src = src.transpose(0, 1) # by transpose, [120 (given seq), batch, ninp]
        tgt = tgt.transpose(0, 1) # by transpose, [24 (predict seq), batch, 72 (3J)]

        # src and tgt are now (T, B, E)
        if max_len is None:
            max_len = tgt.shape[0]

        projected_src = self.encoder(src) * np.sqrt(self.ninp) # why add np.sqrt? [seq, batch, ninp]
        pos_encoded_src = self.pos_encoder(projected_src) # [seq, batch, ninp]
        encoder_output = self.transformer_encoder(pos_encoded_src) # [seq, batch, ninp] encoder output

        if self.training:

            # Create mask for training -> mask away future
            tgt_mask = self._generate_square_subsequent_mask(tgt.shape[0]).to(
                device=tgt.device,
            ) # [24, 24]

            # Use last source pose as first input to decoder
            # src[-1] shape [batch, 3J] (the last pose of the 120 sequence, for all batches) -> unsqueeze: [1, 64, 72]
            # tgt[:-1] shape [24-1, 64, 72]
