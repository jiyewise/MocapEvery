...
def render_cone(p1, p2, r=0.1, color=[1.0, 0.5, 0.0], offset=0.02):
	glColor(color)
	# Create a vector that points from p1 to p2
	direction = np.array(p2) - np.array(p1)
	normalized_dir = direction / np.linalg.norm(direction)
	# Offset p1 and p2 along the direction vector
	p1 = np.array(p1) + offset * normalized_dir
	p2 = np.array(p2) - offset * normalized_dir
	height = np.linalg.norm(p2 - p1)  # height of the cone
	slices = 20  # number of subdivisions around the z-axis
	stacks = 1  # number of subdivisions along the z-axis
	# Calculate the angle between the z-axis and the direction vector
	angle = np.arccos(normalized_dir[2])
	# Calculate the axis of rotation for the cone
	axis = np.cross([0, 0, 1], normalized_dir)
	# Set up the OpenGL transformation matrix to position and orient the cone
	glPushMatrix()
	glTranslate(*p1)
	glRotate(np.degrees(angle), *axis)
	# Create a new quadric object and render the cone
	quadric = gluNewQuadric()
	gluCylinder(quadric, r, r*0.2, height, slices, stacks)
...
def render_camera_pyramid(T, scale=0.1, color=[0,0,0,1], line_width=1.0):
...
def render_quad_2D_texture(
	...
	# # window_height = texHeight
	# # # frame_height = 200
	# # # frame_width = 200
	# # window_width = texWidth
