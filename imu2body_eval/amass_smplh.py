...
def create_motion_from_amass_bdata(bdata, bm, skel_with_offset=None, betas=None, load_motion=True, ):
	# print("called")        
...
def create_motion_from_amass_data(filename, bm, skel_with_offset=None, default_beta=True, load_motion=True, process=True):
	# print("called")        
	try:
	...
	if skel_with_offset is None:
		...
		skel, offset = deepcopy(skel_with_offset)
	# print(bdata.files)
	if "mocap_framerate" not in bdata.files:
		return None
	fps = float(bdata["mocap_framerate"])    
	root_orient = bdata["poses"][:, :3]  # controls the global root orientation (frame, 3)
	pose_body = bdata["poses"][:, 3:66]  # controls body joint angles (frame, 63)
