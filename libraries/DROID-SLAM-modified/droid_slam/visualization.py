...
def create_camera_actor(g, scale=0.03):
...
def droid_visualization(video, queue, device="cuda:0", pcd_save_path=""):
    ...
    # droid_visualization.filter_thresh = 0.005*12 # default 0.005
    droid_visualization.filter_thresh = 0.1 # default 0.005


    droid_visualization.pointcloud_data = None 
    droid_visualization.queue = queue
    ...
    def animation_callback(vis):
        ...
        with torch.no_grad():
            ...
            pts_result = np.zeros(shape=(1,3))
            clr_result = np.zeros(shape=(1,3))
            # print(f"init points: {pts_result.shape}")
            ...
            for i in range(len(dirty_index)):
                ...
                # print(f"update pointcloud {pts_result.shape} {clr_result.shape}")
            ...
            pointcloud_data = pts_result, clr_result
            droid_visualization.queue.put(pointcloud_data)
    ...
    vis.get_render_option().load_from_json("/home/jiye/Desktop/MocapEvery/libraries/DROID-SLAM/misc/renderoption.json")


    vis.run()
# print("call stop event")
# vis.destroy_window()
